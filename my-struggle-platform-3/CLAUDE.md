...
0. **Follow AUTOPILOT.md.** Every session runs the audit → gap-report → build → verify loop against the element checklists in `/requirements` until 100% complete. The gap report is rewritten every run.
