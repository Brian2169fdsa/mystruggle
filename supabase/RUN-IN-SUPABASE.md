# Run this in Supabase — complete database setup

One file. Creates the entire My Struggle database — every table, row-level
security policy, the employer-as-account model, and the de-identified outcomes
plane. **Validated end-to-end on PostgreSQL 16, applied as a single
transaction** (the same way the Supabase SQL editor runs it).

## How to run it (3 minutes, no terminal needed)

1. Open your project → **SQL Editor** → **New query**.
2. Copy the **entire SQL block below** (everything between the ```sql fences).
3. Paste it in and click **Run**.
4. You should see `Success. No rows returned`. Done — accounts, community,
   giving, continuum of care, notifications, events, reports, and outcomes are
   all live and secured.

> It assumes the standard Supabase surface (the `auth` schema, `auth.uid()`,
> and the `anon` / `authenticated` / `service_role` roles) — all of which every
> Supabase project already has. Nothing to set up first.

## What it creates (verified)

- **41 tables** across accounts, community, giving, learning, continuum of care,
  engagement (notifications / events / RSVPs / blocks / reports), and the ad product.
- **142 row-level-security policies** — members see only their own private data,
  staff access is consent-gated and center-scoped, the community respects blocks,
  and the licensed research plane exposes zero identifiers.
- **`user_role` = member, mentor, staff, employer** — employers are accounts
  (`role='employer'` + `company`), and `job_posts` points at `profiles`.
- **Outcomes data product** — `analytics` + `licensed` schemas, 4 matviews, and a
  `licensed_research` role (k≥11 suppression; no identifiers). Infrastructure only
  — do not grant an external party this role until legal sign-off.

## After it runs

- **Rotate the keys** you pasted in chat (Supabase → Settings → API) before
  go-live — treat the current ones as burned.
- The app still runs on its in-memory store until we flip `DATA_BACKEND=supabase`
  and wire the routes (next step). This file gets the *database* ready.
- Optional demo data lives in `supabase/seed.sql` (paste separately if you want
  sample rows) — but real signups will populate it on their own.
